...
func BenchmarkGetEventsInTimeRange(b *testing.B) {
	...
	var res []*Event
	for i := 0; i < b.N; i++ {
		res, _ = GetEventsInTimeRange(index, mainTime, beforeDur, afterDur)
	}
	_ = res
}
...
func BenchmarkBuildAnswer(b *testing.B) {
	...
	var err error
	for i := 0; i < b.N; i++ {
		answer, err = BuildAnswer(mainEvent, index, "events.jsonl", req, rules)
		if (err != nil) {
			b.Fatal(err)
		}
	} 
...
func BenchmarkBuildTimeline(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		timeline = BuildTimeline(mainEvent, contextBefore, contextAfter, userEvents, fileEvents, destinationEvents)
	} 
	_ = timeline
...
func BenchmarkGenerateMarkdownCard(b *testing.B) {
	...
	answer, err := BuildAnswer(mainEvent, index, "events.jsonl", req, []Rule{})
	if (err != nil) {
	...
	var str string
	for i:=0; i<b.N; i++ {
		str = GenerateMarkdownCard(mainEvent, &answer, index, 50)
	}
	_ = str
}
...
func BenchmarkGenerateDOTGraph(b *testing.B) {
	...
	var dot string
	for i:=0; i<b.N; i++ {
		dot, _ =GenerateDOTGraph(&answer, index)
	}
	_ = dot
}
