...
func TestGetEventsInTimeRangeValid(t *testing.T) {
	...
	eventsInTimeRange, err := GetEventsInTimeRange(index, mainTime, "10m", "5m")
...
func TestGetEventsInTimeRangeNoEvents(t *testing.T) {
	...
	eventsInTimeRange, err := GetEventsInTimeRange(index, mainTime, "20h", "1h")
	require.NoError(t, err)
...
func TestGetEventsInTimeRangeBorders(t *testing.T) {
	...
	eventsInTimeRange, err := GetEventsInTimeRange(index, mainTime, "5m", "5m")
	require.NoError(t, err)
...
func TestGetEventsInTimeRangeInvalidMainTime(t *testing.T) {
	...
	_, err := GetEventsInTimeRange(index, mainTime, "5m", "5m")
	require.Error(t, err)
...
func TestGetEventsInTimeRangeInvalidDuration(t *testing.T) {
	...
	_, err := GetEventsInTimeRange(index, mainTime, "1day", "5m")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "Задан неверный формат --before:")

	_, err = GetEventsInTimeRange(index, mainTime, "5m", "1day")
	require.Error(t, err)
